...
public class SlidingRootNavLayout extends FrameLayout implements SlidingRootNav {
    ...
    private View actionBarToggleView;
    ...
    public void setActionBarToggleView(View actionBarToggleView) {
        this.actionBarToggleView = actionBarToggleView;
    }
    ...
    private boolean shouldBlockClick(MotionEvent event) {
        ...
        if (rootView != null && isMenuOpened()) {
            ...
            if (actionBarToggleView != null && isMenuOpened()) {
                int x = Math.round(event.getX());
                int y = Math.round(event.getY());

                Rect rect = new Rect();
                actionBarToggleView.getGlobalVisibleRect(rect);

                int viewLeft = rect.left;
                int viewRight = rect.right;
                int viewTop = rect.top;
                int viewBottom = rect.bottom;

                if(toolbar!=null){
                    Rect rectToolbar = new Rect();
                    toolbar.getGlobalVisibleRect(rectToolbar);

                    viewLeft = rectToolbar.left;
                    viewRight = rect.right+(actionBarToggleView.getWidth()/2);
                    viewTop = rectToolbar.top-(actionBarToggleView.getHeight()/2);
                    viewBottom = rectToolbar.bottom+((actionBarToggleView.getHeight()/2));
                }
